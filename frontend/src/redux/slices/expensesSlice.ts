  ...
  async (
    {
      ...
      limit = 50,
      period = "today",
    }: { page?: number; limit?: number; period?: string },
    { rejectWithValue }
  ) => {
    try {
      const response = await expenseService.getExpenses(page, limit, period);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchExpenses.fulfilled, (state, action) => {
        ...
        // Calculate totals
        const total = action.payload.data.reduce(
        ...
        // Calculate by type
        const byType = action.payload.data.reduce(
          (acc: Record<ExpenseType, number>, expense: Expense) => {
      ...
      .addCase(createExpense.fulfilled, (state, action) => {
        ...
        state.expensesByType[action.payload.type] =
          (state.expensesByType[action.payload.type] || 0) +
      ...
      .addCase(updateExpense.fulfilled, (state, action) => {
        ...
        if (index !== -1) {
          ...
          state.expensesByType[oldExpense.type] =
            (state.expensesByType[oldExpense.type] || 0) - oldExpense.amount;
      ...
      .addCase(deleteExpense.fulfilled, (state, action) => {
        ...
        if (deletedExpense) {
          ...
          state.expensesByType[deletedExpense.type] =
            (state.expensesByType[deletedExpense.type] || 0) -
